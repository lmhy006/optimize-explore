...
% 待第 1 章摘要加入后，删除下一行，使本节自动编号为第 2 章。
\setcounter{section}{1}

\section{引言}
\label{sec:intro}

奈奎斯特采样定理指出，要无失真地重建带限信号，采样率不得低于信号最高频率的两倍。然而在许多实际场景中，信号本身具有稀疏性或可压缩性，传统均匀采样方式获取了大量冗余数据，采样与存储成本高昂。压缩感知理论表明，对于稀疏信号，可以通过远低于奈奎斯特率的线性测量以高概率精确恢复，其核心在于利用信号的稀疏结构。
...
\]
但该问题是 NP-难的组合优化问题，难以大规模精确求解。L1 凸松弛将 L0 范数替换为 L1 范数，得到凸优化问题，从而能够高效求解，并在一定条件下保证恢复精度。

在现有课程作业或朴素实现中，常见以下几类简化缺陷：
\begin{itemize}
\item 直接对正则参数 $\lambda$ 二分搜索，忽略残差函数 $r(\lambda)$ 非单调带来的失效风险；
...
\paragraph*{解析伪逆闭式解}
在欠定情形 $M<n$ 且 $A$ 行满秩时，等式约束 $Ax=y$ 有解，问题 \eqref{eq:L2-baseline} 可退化为求满足 $Ax=y$ 的最小范数解：
\begin{equation}
\label{eq:l2-pinv}
\hat{x}_{L2}=A^T(AA^T)^{-1}y.
\end{equation}
该解可通过解析伪逆直接计算，无需迭代优化。相比原始方案中的梯度投影迭代，解析伪逆具有以下优点：
\begin{itemize}
\item 闭式求解，计算开销低；
\item 不引入迭代收敛误差，保证对比公平；
\item 在仿真噪声水平下，伪逆解通常满足残差约束 $\|A\hat{x}_{L2}-y\|_2\le\delta$。
\end{itemize}
...
\item 使用第~\ref{sec:l2-baseline} 节的解析伪逆方法求解 L2 基线
\[
\hat{x}_{L2}=A^T(AA^T)^{-1}y;
\]
